...
_Public lands and waters - costs purchased by FWS_:
```{r n_intro}
formatted_number3 <- formatC(sum(dat$pub_fws_purchase_cost), format = "d", big.mark=",")
...
_Easements - costs purchased by FWS_: 
```{r, eval=FALSE}
...
### Information
Fiscal Year 2019 Acquisitions and Disposals from 10/1/2018 through 9/30/2019.
...
Public lands and waters - acres purchased by FWS:
...
Public lands and waters - costs purchased by FWS:
```{r}
tags$p(
  style = "font-size:1.5em; font-weight:700",
  sum(fy$pub_fws_purchase_acres)
)
```

<hr>

Easements - acres purchased by FWS:
```{r}
tags$p(
  style = "font-size:1.5em; font-weight:700",
  sum(fy$ease_fws_purchase_acres)
)
```
