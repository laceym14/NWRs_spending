...
natmon_nwrs <- read.csv("table_10_nwrs_unit.csv")
natmon_non_nwrs <- read.csv("table_10_non_unit.csv")
...
p <- nm %>%
  ...
  plot_ly(
    x= ~nm$unit_name, 
    y = ~nm$total_monument_acres) %>%
  add_bars(p, x = ~nm$unit_name, y = ~nm$total_monument_acres, data = ~nm, inherit = TRUE)
})
...
renderPlotly({
plot_ly(natmon_nwrs, labels = ~ natmon_nwrs$monument_in_nwrs, values = ~ natmon_nwrs$total_monument_acres, type = 'pie',
        textposition = 'inside', textinfo = 'label+acres') %>%
  layout (title='Nat Mon in wildlife refuge', 
          showlegend = TRUE)
})
