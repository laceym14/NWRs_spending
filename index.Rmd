...
notes: 
include easements;
good to have FY 19 but also overall refuge report-which is tab 1- to note changes;
make sure waterfowl production areas are defined;
add refuge locations - Version2
...
_Maybe add easement information here instead of other agencies?_ Acres acquired by other agencies (DOE, AF, R, etc.) for which the Service has *some* management responsibilities:
```{r n_intra}
...
### Background
```{r}
img(src = "redwolves.png", height = 340, width = 600)
```
